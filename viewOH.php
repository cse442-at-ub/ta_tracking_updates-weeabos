...
$e = strval($_POST['old_end_time']);
$oldend = date('Y/m/d H:i:s',strtotime($a . " " . $e));
$startdt = date('Y/m/d H:i:s',strtotime($a . " " . $b));
$enddt = date('Y/m/d H:i:s', strtotime($a . " " . $c));
$oldend = date('Y/m/d H:i:s', strtotime($a . " " . $c));
$course = $_SESSION['courseSelected'];
$update = $conn->prepare("UPDATE office_hours SET start_time = ?, expected_end = ?, JustDate = ?, actual_end = ? WHERE email = ? AND start_time = ? AND expected_end = ?");
$update->bind_param("ssssss", $startdt, $enddt, $a, $enddt, $d, $oldstart, $oldend);
$update->execute();
echo $update->error;
